    ...
    <div className="catalog-admin">
      <div className="catalog__container">
        ...
        <div className="catalog-admin__header">
          ...
          {/* Кнопка теперь работает и ведет на страницу добавления */}
          <button 
            className="add-btn" 
                ...
                <div className="product-card__info">
                  ...
                  <div className="product-card__section product-card__section--status">
                    <div>
                      ...
                      <p className={`product-card__status ${!camera.inStock ? "product-card__status--out" : ""}`}>
                        {camera.status}
                      </p>
                    </div>
                    <div className="menu-wrapper">
                      <button className="product-card__menu" onClick={() => handleMenuClick(camera.id)}>
                        <MoreVertical size={24} />
                      </button>
                      {activeMenu === camera.id && (
                        <div className="dropdown-menu">
                          <button className="dropdown-menu__item" onClick={() => handleEdit(camera)}>
                            Редактировать
                          </button>
                          <button className="dropdown-menu__item dropdown-menu__item--danger" onClick={() => handleDelete(camera)}>
                            Удалить публикацию
                          </button>
                        </div>
                      )}
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal__body">
              ...
              <div className="modal__buttons">
                <button className="modal__btn modal__btn--primary" onClick={() => handleStatusChange(true)}>В наличии</button>
                <button className="modal__btn modal__btn--secondary" onClick={() => handleStatusChange(false)}>Не в наличии</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showDeleteModal && (
        <div className="modal-overlay" onClick={() => setShowDeleteModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal__body">
              <p className="modal__text">Вы уверены, что хотите удалить публикацию?</p>
              <div className="modal__buttons">
                <button className="modal__btn modal__btn--danger" onClick={confirmDelete}>Удалить</button>
                <button className="modal__btn modal__btn--secondary" onClick={() => setShowDeleteModal(false)}>Отмена</button>
              </div>
            </div>
          </div>
        </div>
